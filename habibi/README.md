...
| [dev.sh](dev.sh)                                                               | дирижёр локальной среды: `init`, `up`, `down`, `logs` — поднимает девконтейнер, движок, вотчер расширения, бенч и vite; `check` — диагностика цепочки браузер-бенч-движок-БД по звеньям |
