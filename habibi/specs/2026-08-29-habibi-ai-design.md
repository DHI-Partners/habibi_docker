...
| --- | --- | --- |
| **Движок** `habibi_ai_engine` | Directus 12 + Postgres 18 + расширение `ai` | один на инсталляцию |
...
Containerfile           FROM directus/directus:12 + собранное расширение
...
**Версия SDK и host у расширения.** Расширение собрано под
`@directus/extensions-sdk` 17.0.4 и объявляет `"host": "^10.10.0"`, а целевой образ —
Directus 12.3.1 при актуальном SDK 18.0.4. Это прыжок через две мажорные версии, поэтому
обновление SDK и `host` — обязательная часть переноса, а не проверка на всякий случай.
Если API расширений разошлось настолько, что endpoint не собирается, запасной вариант —
зафиксировать образ на `directus/directus:11` и вернуться к обновлению отдельной задачей.
