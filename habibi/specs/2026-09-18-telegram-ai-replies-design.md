...
- `is_automated = 1`, если выполняется одно из:
  - `frappe.flags.in_telegram_update` (ответы обработчиков бота, `/start`,
    аутентификация);
  - вызывающий передал `automated=True` в функцию отправки
    (`client.send_message` / `send_file` / `send_voice`,
    `user_client.send_message`). Параметр, а не глобальный флаг: отправка
    Notification уходит отдельной фоновой задачей, и флаг туда не доехал бы.
    `overrides.notification` передаёт `automated=True`.
- MTProto (`handlers.account.log_message`) — `is_automated` из того же
  параметра; исходящие, увиденные при синхронизации (отправлены с телефона),
  его не несут и остаются `is_automated = 0`.
...
- Под redis-блокировкой пары
  ...
  5. если за время генерации появились новые входящие — повторяет шаги
     1–4 в той же задаче (не больше 5 раундов). Новую задачу не ставит:
     её `job_id` совпал бы с выполняющейся, и дедупликация её отбросила бы.

Пока задача ждёт в очереди или выполняется, дубль с тем же `job_id` не
создаётся; новое сообщение подбирается на шаге 5. Выборка ожидающих берёт
последние 20 входящих и отбрасывает несвежие (§3.1), поэтому на первом
сообщении старая история в движок не уходит.
