...
Всё, что описано ниже до конца этой части — руками, шаг за шагом, — автоматизирует
[dev.sh](dev.sh): `dev.sh init` разово поднимает девконтейнер, `bench init`, сайт
`dev.localhost` и подключает соседние `habibi_ui`/`habibi_ai`, а `dev.sh up` дальше
поднимает всё сразу вместе с движком ИИ, вотчером расширения и vite. Ручной путь
по-прежнему нужен, когда `dev.sh` не подходит: свой бенч без ИИ-модуля, другое имя
сайта, отладка самого скрипта или шага, который в нём упал.
